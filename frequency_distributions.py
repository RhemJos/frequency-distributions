...
# data_file = pd.read_csv(csv_path, sep=';', encoding='latin1')

# output_dir = "frequency_charts"
# os.makedirs(output_dir, exist_ok=True)

# for column in data_file.columns[6:]:
#     frequency = data_file[column].value_counts(dropna=True)
#     if len(frequency) == 0 or frequency.sum() == 0:
#         continue
...
#     plt.figure(figsize=(10, 6))
#     bars = plt.bar(frequency.index.astype(str), frequency.values, color='green', alpha=0.7)
#     # frequency.plot(kind='bar', color='green', alpha=0.7)
#     plt.title(f'Distribución de Frecuencias: {column}')
#     plt.xlabel("Respuesta")
#     plt.ylabel("Frecuencia")
...
#     plt.savefig(os.path.join(output_dir, f"{column}_freq.png"))
#     print(f"Guardado: Gráfico de {column}")
#     plt.close()
